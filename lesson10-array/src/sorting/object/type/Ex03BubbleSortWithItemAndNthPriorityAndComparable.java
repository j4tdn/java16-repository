...
public class Ex03BubbleSortWithItemAndNthPriorityAndComparable {
	...
	private static void bubbleSort(Item[] elements, NullOption no) {
		for (int round = 0; round < elements.length; round++) {
			for (int i = 0; i < elements.length - round - 1; i++) {
				 ...
				 * elements[i] = elements[i+1]; elements[i+1] = tmp; }else if(i1.getStoreId() -
				 * i2.getStoreId() == 0) { if(i1.getId() - i2.getId() > 0) { Item tmp =
				 * elements[i]; elements[i] = elements[i+1]; elements[i+1] = tmp; } }
				 */
	...
	public static int compareTo1(Item i1, Item i2) {
