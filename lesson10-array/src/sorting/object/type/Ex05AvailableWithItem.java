...
public class Ex05AvailableWithItem {
	public static void main(String[] args) {
		Item[] items = getItems();

		// comparable (support for one sort function)
		// Arrays.sort(X[]) --> implements Comparable<X> --> x1.compare(x2) ...
		// default: if(compareTo(that)>0 --> swap
		// 1.Sorting item descending by storeID
		Arrays.sort(items);
		ArrayUtils.printf("1.Sorting item descending by storeId", items);

		// comparator
		// 2.Sorting item store(asc), price(desc)
		...
		ArrayUtils.printf("2.Sorting item store(asc), price(desc)", items);
