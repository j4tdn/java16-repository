	...
	private static void bubbleSort(Item[] element, SortType st, NullOption no) {
		for (int i = 0; i < element.length; i++) {
			for (int j = 0; j < element.length - i - 1; j++) {
				...
				if(SortType.ASC.equals(st)) {
					if(element[j].getStoreId() - element[j+1].getStoreId() > 0 || (element[j].getStoreId() - element[j+1].getStoreId() == 0 && element[j].getId() - element[j+1].getId() > 0 )) {
						Item tmp = element[j];
						element[j] = element[j+1];
						element[j+1] = tmp;
					}
