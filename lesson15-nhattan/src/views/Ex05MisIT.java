...
public class Ex05MisIT {
	private static final String pathname = "text\\mistIT.txt";
	
	public static void main(String[] args) {
		
		List<List<Integer>> lines = getLines(pathname);
		...
		for (Entry<Integer, Contestant> entry : list) {
			Contestant contestant = entry.getValue();
			if (contestant.compareTo(winner) == 0) {
				System.out.print("Winner: " + contestant);
			}
		}
	}
	
	private static List<List<Integer>> getLines(String pathname) {
		File file = new File(pathname);
		
		List<String> stringList = FileUtils.read(file);
		stringList.forEach(System.out::println);
		
		List<List<Integer>> lines = new ArrayList<>();
		for (String line : stringList) {
			List<Integer> numbersInLine = convert(line);
			lines.add(numbersInLine);
		}
